...
const char * const LIBRA_CONF_FILENAME = "libra.conf";
const char * const LIBRA_PID_FILENAME = "librad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "librad -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Libra";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LIBRA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", LIBRA_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Libra Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Libra Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Libra Core developers";
    }
