...
pkglibexecdir = $(libexecdir)/libra
...
CONFIG_HEADER = $(top_builddir)/src/config/libra-config.h
CONFIG_CLEAN_FILES = liblibraconsensus.pc share/setup.nsi \
...
CONFIG_CLEAN_VPATH_FILES = contrib/filter-lcov.py \
	test/functional/test_runner.py test/util/libra-util-test.py \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/liblibraconsensus.pc.in \
	...
	$(top_srcdir)/src/config/libra-config.h.in \
	$(top_srcdir)/test/config.ini.in \
	$(top_srcdir)/test/functional/test_runner.py \
	$(top_srcdir)/test/util/libra-util-test.py \
...
ACLOCAL = ${SHELL} /home/blockchaindeveloper/libra/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /home/blockchaindeveloper/libra/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/blockchaindeveloper/libra/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/blockchaindeveloper/libra/build-aux/missing automake-1.15
...
COPYRIGHT_HOLDERS_FINAL = The Libra Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Libra Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
LIBMEMENV = 
LIBOBJS = 
LIBRA_CLI_NAME = libra-cli
LIBRA_DAEMON_NAME = librad
LIBRA_GUI_NAME = libra-qt
LIBRA_TX_NAME = libra-tx
LIBS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/blockchaindeveloper/libra/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = libra
PACKAGE_BUGREPORT = https://github.com/libra/libra/issues
PACKAGE_NAME = Libra Core
PACKAGE_STRING = Libra Core 0.17.1
PACKAGE_TARNAME = libra
PACKAGE_URL = https://libracore.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/blockchaindeveloper/libra
abs_srcdir = /home/blockchaindeveloper/libra
abs_top_builddir = /home/blockchaindeveloper/libra
abs_top_srcdir = /home/blockchaindeveloper/libra
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/blockchaindeveloper/libra/build-aux/install-sh
...
pkgconfig_DATA = liblibraconsensus.pc
LIBRAD_BIN = $(top_builddir)/src/$(LIBRA_DAEMON_NAME)$(EXEEXT)
LIBRA_QT_BIN = $(top_builddir)/src/qt/$(LIBRA_GUI_NAME)$(EXEEXT)
LIBRA_CLI_BIN = $(top_builddir)/src/$(LIBRA_CLI_NAME)$(EXEEXT)
LIBRA_TX_BIN = $(top_builddir)/src/$(LIBRA_TX_NAME)$(EXEEXT)
LIBRA_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Libra-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/libra.icns
OSX_PLIST = $(top_builddir)/share/qt/Info.plist #not installed
OSX_QT_TRANSLATIONS = da,de,es,hu,ru,uk,zh_CN,zh_TW
DIST_DOCS = $(wildcard doc/*.md) $(wildcard doc/release-notes/*.md)
DIST_CONTRIB = $(top_srcdir)/contrib/libra-cli.bash-completion \
	       $(top_srcdir)/contrib/libra-tx.bash-completion \
	       $(top_srcdir)/contrib/librad.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/libra.ico \
...
COVERAGE_INFO = baseline.info \
  test_libra_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_libra_coverage.info test_libra.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/libra.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Libra-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) test/functional/test_runner.py \
	...
	test/util/libra-util-test.py \
	test/util/data/libra-util-test.json \
...
CLEANFILES = $(OSX_DMG) $(LIBRA_WIN_INSTALLER)
...
src/config/libra-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/libra-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/libra-config.h
$(top_srcdir)/src/config/libra-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/libra-config.h src/config/stamp-h1
liblibraconsensus.pc: $(top_builddir)/config.status $(srcdir)/liblibraconsensus.pc.in
...
$(LIBRA_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(LIBRAD_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(LIBRA_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(LIBRA_CLI_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(LIBRA_TX_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/libra.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Libra-Qt: all-recursive
	$(MKDIR_P) $(@D)
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM)  $(LIBRA_QT_BIN) $@
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Libra-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Libra-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_libra.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_libra -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_libra_filtered.info: test_libra.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_libra_filtered.info
...
#test_libra_coverage.info: baseline_filtered.info test_libra_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_libra_filtered.info -o $@

#total_coverage.info: test_libra_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_libra_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_libra.coverage/.dirstamp:  test_libra_coverage.info
...
#cov: test_libra.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_libra.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
